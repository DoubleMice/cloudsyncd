...
async function startTunnelBackground(options = {}) {
  ...
  const name = tunnelLabel(options);
  const logPath = resolveTunnelLogFile(options);
  const pidfile = resolveTunnelPidFile(options);
  clearStaleTunnelPidfile(pidfile);
  const logFd = fs.openSync(logPath, 'a');
  const args = cloudflaredRunArgs(options);

  print(`Starting Cloudflare Tunnel (${name}) in the background...`);
  ...
  await waitForTunnelStartup(child, logPath);

  if (!fs.existsSync(pidfile)) {
    fs.writeFileSync(pidfile, String(child.pid), { mode: 0o600 });
  }
  print(`  tunnel pid: ${child.pid}  (pidfile: ${pidfile}, logs: ${logPath})`);
}
...
function readAdminToken() {
  ...
  try {
    ...
    throw new Error('Cannot read admin token. Is the server running?');
  }
}

async function fetchAdminJson(route, options = {}) {
  const host = process.env.ADMIN_HOST || '127.0.0.1';
  const port = process.env.ADMIN_PORT || 21900;
  const token = readAdminToken();
  let res;
  try {
    res = await fetch(`http://${host}:${port}${route}`, {
      ...options,
      headers: {
        ...(options.headers || {}),
        'x-admin-token': token,
    ...
    throw new Error(`Cannot reach local admin API at http://${host}:${port}. Is the server running?`);
  }

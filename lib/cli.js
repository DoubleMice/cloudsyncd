...
Tunnel setup:
  ...
  3. cp config/cloudflared.example.yml cloudflared-config.yml
...
function requireTunnelConfig(config) {
  if (!fs.existsSync(config)) {
    throw new Error(`Named tunnel start requires a config file: ${config}. Copy config/cloudflared.example.yml to cloudflared-config.yml, then fill tunnel, credentials-file, and hostname.`);
  }
}
...
async function runServer(argv) {
  ...
  if (cmd === 'share') {
    if (subcmd === 'add') {
      return runNodeScript('scripts/share.js', normalizeShareAddArgs(rest));
    }
    if (subcmd === 'list') return runNodeScript('scripts/share.js', ['--list']);
    if (subcmd === 'clear') return runNodeScript('scripts/share.js', ['--clear']);
    throw new Error('Usage: cloudsyncd server share <add|list|clear>');
  }

  if (cmd === 'devices') return runNodeScript('scripts/devices.js', ['--list']);
  if (cmd === 'revoke') {
    if (!subcmd) throw new Error('Usage: cloudsyncd server revoke <deviceId>');
    return runNodeScript('scripts/devices.js', ['--revoke', subcmd]);
  }
  if (cmd === 'revoke-all') return runNodeScript('scripts/devices.js', ['--revoke-all']);
  if (cmd === 'rotate-key') return runNodeScript('scripts/devices.js', ['--rotate-key']);
  if (cmd === 'rotate-token') return runNodeScript('scripts/devices.js', ['--rotate-token']);

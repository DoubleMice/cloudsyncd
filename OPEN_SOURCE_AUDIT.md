...
3. 已加入设备撤销、全部撤销、主密钥轮换和管理 Token 轮换。
...
- `data/state.json`: 主密钥、管理 Token、已配对设备清单
- `data/.admin-token`: 本地管理 Token 副本
- `shared/`: 运行态共享载荷
- `.env*`: 本地环境变量
...
Cloudflare Tunnel 配置只转发客户端端口，不转发管理端。公网域名访问 `/admin`、`/admin.js`、`/api/local/*` 不会命中管理服务。远程管理应通过 SSH 端口转发访问 `127.0.0.1:21900`。

不要在没有额外网络控制的情况下把 `ADMIN_HOST` 改成 `0.0.0.0`。

## 仍需接受的风险

- 主密钥和管理 Token 明文保存在本机 `data/state.json`，依赖主机文件权限和 `.gitignore` 隔离。
- 浏览器客户端会把主密钥保存在 IndexedDB，浏览器账户或终端被入侵时可能泄露。
- 所有设备共享同一个主密钥，没有每设备独立内容密钥或单设备 ACL。
- `shared/` 下所有非隐藏文件都会对已配对设备可见。
...
`cloudflared-config.yml` 是当前私人部署配置，包含：

- 隧道 UUID
- 本机 `~/.cloudflared/...json` 凭证路径
- `sync.example.com` 入口域名

这些值不等同于凭证本体，但属于部署信息。私有仓库可保留；公开仓库前应改为模板或移除。
